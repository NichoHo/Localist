...
fputcsv($out, ['name', 'category', 'city', 'region', 'address', 'phone', 'website', 'email', 'description', 'lat', 'lng']);
...
while ($rows < $target) {
    ...
    fputcsv($out, [$name, $cat, $city, $region, $address, $phone, $website, $email, $desc, $lat, $lng]);
    $rows++;
}
fclose($out);
