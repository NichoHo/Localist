<x-public-layout>
    @php
        ...
        $openCount = cache()->remember('home.open_now_count', 300, fn () => \App\Models\Business::published()->get(['id', 'hours'])->filter->isOpenNow()->count());
    @endphp
    ...
    <section class="mx-auto max-w-[76rem] px-4 pb-10 pt-14 sm:px-6 sm:pt-20">
        <div class="max-w-2xl">
            <h1 class="font-display text-4xl font-bold leading-[1.05] text-ink sm:text-5xl lg:text-6xl">Someone who can come out <mark class="bg-accent px-1.5 text-on-accent">today</mark>.</h1>
            <p class="mt-5 max-w-xl text-lg leading-relaxed text-ink-muted">{{ number_format($businessCount) }} local plumbers, electricians, cleaners, tutors and more across {{ $cityCount }} Malaysian cities. Free to search, free to call.</p>
            ...
            <form action="{{ route('search') }}" method="get" class="mt-8 flex max-w-xl flex-col overflow-hidden rounded border-2 border-ink bg-surface sm:flex-row">
                ...
                <input id="hero-search" type="search" name="q" placeholder="Burst pipe, no hot water, aircond leaking"
    ...
    <section class="mx-auto max-w-[76rem] px-4 py-16 sm:px-6 sm:py-20">
        <h2 class="font-display text-2xl font-bold text-ink sm:text-3xl">Browse by trade</h2>
    ...
    {{-- CTA: for businesses. Solid yellow, the one other place on the page
         ...
         real photo of trade work earns the other half of the panel instead
         of leaving it a flat color block; desaturated so it sits inside the
         ink/paper palette instead of fighting it with its own colors. --}}
    {{-- Photo: Timur Shakerzianov via Unsplash, free license, no attribution required. --}}
    <section class="mx-auto max-w-[76rem] px-4 sm:px-6">
        <div class="grid overflow-hidden rounded-md bg-accent sm:grid-cols-2">
            ...
            <div class="min-h-56 sm:order-1">
                <img src="https://images.unsplash.com/photo-1676210134188-4c05dd172f89?fm=jpg&amp;q=80&amp;w=1200&amp;h=800&amp;fit=crop&amp;auto=format"
                    alt="A tradesperson repairing pipework under a sink"
